...
class Minesweeper:
    ...
    def __init__(self, master: tk.Tk, rows: int = 10, cols: int = 10, mines: int = 10) -> None:
        ...
        # Add Restart Button
        self.restart_button = tk.Button(self.status_frame, text="Restart", command=self.restart_game, width=10)
    ...
    def create_widgets(self) -> None:
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                button.bind("<Button-3>", lambda e, r=r, c=c: self.on_right_click(r, c)) # Bind right-click
                ...
                for dr in [-1, 0, 1]:
                    for dc in [-1, 0, 1]:
                        if dr == 0 and dc == 0:
                            continue
                        nr, nc = r + dr, c + dc
                        if 0 <= nr < self.rows and 0 <= nc < self.cols and (nr, nc) in self.mine_locations:
                            count += 1
    ...
    def reveal_cell(self, r: int, c: int) -> None:
        ...
        if self.game_over or (r, c) not in self.buttons or self.buttons[(r, c)]['state'] == tk.DISABLED:
            return
        ...
        while stack:
            ...
            if adjacent_count > 0:
                ...
                button.config(disabledforeground=self.NUMBER_COLORS.get(adjacent_count, 'black'))
            else:
                ...
                for nr, nc in self.get_neighbors(current_r, current_c):
                    if (nr, nc) in self.buttons:
                        ...
                        if neighbor_button['state'] == tk.NORMAL and neighbor_button['text'] != '🚩':
                            stack.append((nr, nc))

        # Check win condition
        if not self.game_over and self.cells_revealed == self.rows * self.cols - self.mines:
            self.win_game()
    ...
    def on_click(self, r: int, c: int) -> None:
        ...
        if button['state'] == tk.DISABLED or button['text'] == '🚩': # Do nothing if already revealed or flagged
            return
    ...
    def lose_game(self, clicked_r: int, clicked_c: int) -> None:
        ...
        for r, c in self.mine_locations:
            if (r,c) in self.buttons: # Check if button exists
                ...
                if r == clicked_r and c == clicked_c:
                     button.config(text='💥', bg='red', state=tk.DISABLED, disabledforeground='black') # Clicked mine
                elif button['text'] != '🚩': # Correctly flagged mines remain flagged
                     button.config(text='💣', bg='lightgrey', state=tk.DISABLED, disabledforeground='black') # Other mines
        ...
        for (r, c), button in self.buttons.items():
            if button['text'] == '🚩' and (r, c) not in self.mine_locations:
                button.config(text='❌', state=tk.DISABLED, disabledforeground='red') # Incorrect flag
            elif button['state'] == tk.NORMAL: # Disable remaining buttons that are not mines
                 button.config(state=tk.DISABLED)
        if self.show_messages:
            messagebox.showerror("Game Over", "Game over! You stepped on a mine.") # Show message after updating UI
    ...
    def win_game(self) -> None:
        ...
        for r, c in self.mine_locations:
             if (r,c) in self.buttons and self.buttons[(r, c)]['state'] == tk.NORMAL: # Only flag unrevealed mines
                 self.buttons[(r, c)].config(text='🚩', fg='green', state=tk.DISABLED, disabledforeground='green')
        # Disable any remaining normal buttons (shouldn't be any if win condition is met correctly)
        for button in self.buttons.values():
            if button['state'] == tk.NORMAL:
                button.config(state=tk.DISABLED)
        if self.show_messages:
            messagebox.showinfo("Win", "Congratulations! You won!") # Show message after updating UI

    def update_status(self) -> None:
        """Update the status bar with current game information"""
        # Update status bar label
        remaining_mines = self.mines - self.flags
        self.status_label.config(text=f"Total Mines: {self.mines} | Remaining: {remaining_mines}")
