...
class OutputLogger:
    def __init__(self, master):
        ...
        self.log_text = scrolledtext.ScrolledText(
            self.log_window, wrap=tk.WORD, state="disabled"
        )
...
class MinePlayer:
    def __init__(self, game_instance, logger):
        ...
        self.board_state = (
            {}
        )  # Used to store our knowledge of the board: 'H'idden, 'R'evealed, 'F'lagged
                    ...
                    try:
                        ...
                        if adjacent_mines_count > 0:
                            ...
                            hidden_neighbors = [
                                n
                                for n in neighbors
                                if self.board_state[n] in ["H", "F"]
                            ]
                            unflagged_hidden_neighbors = [
                                n
                                for n in hidden_neighbors
                                if self.board_state[n] == "H"
                            ]

                            if (
                                len(hidden_neighbors) == adjacent_mines_count
                                and len(unflagged_hidden_neighbors) > 0
                            ):
                                for nr, nc in unflagged_hidden_neighbors:
                                    self.logger.log(
                                        f"Strategy 1: Flagging mine at ({nr}, {nc}) based on ({r}, {c})={adjacent_mines_count}"
                                    )
                                    self.game.on_right_click(
                                        nr, nc
                                    )  # Simulate right-click to flag
                                    self.board_state[(nr, nc)] = (
                ...
                if self.board_state[(r, c)] == "R":
                    ...
                    try:
                        ...
                        if adjacent_mines_count > 0:
                            neighbors = self.get_neighbors(r, c)
                            flagged_neighbors = [
                                n for n in neighbors if self.board_state[n] == "F"
                            ]
                            hidden_unflagged_neighbors = [
                                n for n in neighbors if self.board_state[n] == "H"
                            ]

                            if (
                                len(flagged_neighbors) == adjacent_mines_count
                                and len(hidden_unflagged_neighbors) > 0
                            ):
                                for nr, nc in hidden_unflagged_neighbors:
                                    self.logger.log(
                                        f"Strategy 2: Clicking safe cell ({nr}, {nc}) based on ({r}, {c})={adjacent_mines_count}"
                                    )
    ...
    def auto_play_step(self):
        ...
        if not made_move:
            ...
            if not self.make_random_guess():
                ...
                hidden_cells = [
                    pos for pos, state in self.board_state.items() if state == "H"
                ]
                flagged_cells = [
                    pos for pos, state in self.board_state.items() if state == "F"
                ]
                if len(hidden_cells) == 0 and len(flagged_cells) == self.game.mines:
                    self.logger.log(
                        "All non-mine cells are revealed, or all mines are flagged."
                    )
                else:
